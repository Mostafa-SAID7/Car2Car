    ...
    <div v-if="product" class="mt-7 grid gap-10 lg:grid-cols-[0.95fr_1.05fr] lg:gap-16">
      <div>
        <div class="relative aspect-square overflow-hidden rounded-[28px] border border-white/10 bg-[#181c22]">
          ...
          <span class="absolute left-5 top-5 rounded-full bg-[#F5A710] px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.12em] text-[#0B0D10]">{{ product.brand }}</span>
        ...
        <div class="mt-3 grid grid-cols-2 gap-3">
          <button v-for="(image, index) in product.images" :key="image" class="aspect-[1.5] overflow-hidden rounded-xl border-2 bg-[#181c22]" :class="selectedImage === index ? 'border-[#F5A710]' : 'border-transparent opacity-60'" :aria-pressed="selectedImage === index" @click="selectedImage = index"><img :src="image" :alt="`${product.name} view ${index + 1}`" class="h-full w-full object-cover mix-blend-screen" /></button>
        </div>
      </div>

      <div class="flex flex-col justify-center">
        <div class="flex items-center gap-2 text-xs text-[#F5A710]"><Star :size="15" fill="currentColor" stroke="none" /><strong>{{ product.rating }}</strong><span class="text-[#8E96A3]">({{ product.reviewCount }} reviews)</span></div>
        <h1 class="display-font mt-4 max-w-xl text-4xl font-bold leading-[0.98] tracking-[-0.06em] text-white sm:text-6xl">{{ product.name }}<span class="text-[#F5A710]">.</span></h1>
        ...
        <p v-else class="mt-5 flex items-center gap-2 text-xs font-semibold" :class="compatible ? 'text-[#58D5E6]' : 'text-[#F5A710]'"><Check :size="16" /> {{ compatible ? `Fits your ${vehicleStore.vehicle.make} ${vehicleStore.vehicle.model}` : `Check fitment for your ${vehicleStore.vehicle.make}` }}</p>
      </div>
    </div>
