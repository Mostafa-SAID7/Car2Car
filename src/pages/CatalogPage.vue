...
<template>
  <div class="pb-10 pt-10 sm:pt-14">
    ...
    <div v-else-if="filteredProducts.length" class="space-y-3"><RouterLink v-for="product in filteredProducts" :key="product.id" :to="`/product/${product.slug}`" class="flex items-center gap-4 rounded-2xl border border-white/[0.08] bg-[#14171C] p-3 transition hover:border-white/20 sm:p-4"><div class="h-24 w-24 shrink-0 overflow-hidden rounded-xl bg-[#20252C]"><img :src="product.image" :alt="product.name" class="h-full w-full object-cover opacity-70 mix-blend-screen" /></div><div class="min-w-0 flex-1"><p class="text-[10px] font-bold uppercase tracking-[0.12em] text-[#F5A710]">{{ product.brand }} · {{ product.category }}</p><h2 class="mt-1 truncate text-sm font-semibold text-white">{{ product.name }}</h2><p class="mt-1 text-[11px] text-[#8E96A3]">{{ product.volume }} · {{ product.base }} · {{ product.stock }} in stock</p></div><div class="text-right"><p class="text-sm font-bold text-white">${{ product.price.toFixed(2) }}</p><span class="mt-2 flex items-center justify-end gap-1 text-[11px] text-[#F5A710]"><Star :size="12" fill="currentColor" stroke="none" />{{ product.rating }}</span></div></RouterLink></div>
    <div v-else class="rounded-2xl border border-dashed border-white/10 py-20 text-center"><Search :size="28" class="mx-auto text-[#68707D]" /><h2 class="mt-4 text-lg font-semibold text-white">No fluids found</h2><p class="mt-2 text-sm text-[#8E96A3]">Try a different brand, viscosity, or filter combination.</p><button class="mt-5 text-xs font-bold text-[#F5A710]" @click="query = ''; clearFilters()">Clear search</button></div>
  </div>
</template>
...
.filter-select:focus { border-color: #f5a710; }
.filter-checkbox { height: 15px; width: 15px; accent-color: #f5a710; }
</style>
